...
(function () {
  ...
  function renderCriteres(root) {
    ...
    const trop = selection.size > 2
      ? '<p class="gt-alerte">⚠ Tu as coché ' + selection.size + ' options. ' + REGLE_MAX_DEUX_OPTIONS + '</p>'
      : "";

    root.innerHTML =
      '<h3>Qu\'est-ce qui t\'intéresse ?</h3>' +
      '<p class="gt-hint">' + REGLE_MAX_DEUX_OPTIONS + '</p>' +
      ...
      '<div class="gt-chips">' + chipsVoeu + '</div>' + trop +
  ...
  function construireVoeux() {
    ...
    if (strategie === "lycee") {
      ...
      // Toutes les options d'abord, tous les filets ensuite
      retenus.forEach(function (o) { optionsDe(o).forEach(function (x) { liste.push(x); }); });
